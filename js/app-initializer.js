...
export class AppInitializer {
    ...
    constructor() {
        ...
        this.globalCursorsInitialized = false;
        this.initialCodeSeeded = false;
    }
    ...
    _initCodeEditorManager() {
        ...
        if (this.codeEditorManager && this.codeEditorManager.isInitialized) {
            console.log('[AppInitializer] CodeEditorManager уже инициализирован, пропускаем');
            return;
        }
        
        console.log('[AppInitializer] Инициализация CodeEditorManager...');
        console.log('[AppInitializer] Проверяем Yjs объекты:');
        ...
        console.log('[AppInitializer] - yTextHtml:', !!this.socketService.yTextHtml);
        console.log('[AppInitializer] - yTextCss:', !!this.socketService.yTextCss);
        
        // Проверяем, что все Yjs объекты готовы
        if (!this.socketService.yDoc || !this.socketService.yAwareness || 
            !this.socketService.yTextHtml || !this.socketService.yTextCss) {
            console.log('[AppInitializer] Yjs объекты еще не готовы, отменяем инициализацию');
            return;
        }
    ...
    _subscribeToSocketEvents() {
        ...
        // При синхронизации Yjs: если документы пустые — заполняем из code.txt
        this.socketService.onYjsSynced(async (syncData) => {
            try {
                if (this.initialCodeSeeded) return;
                if (!this.socketService?.yTextHtml || !this.socketService?.yTextCss) return;

                const htmlEmpty = (this.socketService.yTextHtml.length || 0) === 0;
                const cssEmpty = (this.socketService.yTextCss.length || 0) === 0;
                if (!htmlEmpty && !cssEmpty) return;

                const response = await fetch('/code.txt', { cache: 'no-store' });
                if (!response.ok) {
                    console.warn('[AppInitializer] Не удалось загрузить code.txt:', response.status);
                    return;
                }
                const raw = await response.text();

                // Парсим HTML и CSS
                const parsed = this._parseInitialCodeFromText(raw);
                const initialHtml = parsed.html.trim();
                const initialCss = parsed.css.trim();

                if (!initialHtml && !initialCss) return;

                // Транзакционно записываем в Yjs документы
                const ydoc = this.socketService.yDoc;
                if (!ydoc) return;
                ydoc.transact(() => {
                    if (htmlEmpty && initialHtml) {
                        this.socketService.yTextHtml.insert(0, initialHtml);
                    }
                    if (cssEmpty && initialCss) {
                        this.socketService.yTextCss.insert(0, initialCss);
                    }
                }, 'initial-seed');

                this.initialCodeSeeded = true;
                console.log('[AppInitializer] Начальный код загружен из code.txt и применен.');
            } catch (e) {
                console.error('[AppInitializer] Ошибка начальной инициализации кода:', e);
            }
        });
        ...
        this.socketService.onAuth((data) => {
            if (data.success) {
                ...
                // CodeEditorManager будет инициализирован через событие onCodeInitialized
            }
        });
    ...
    /**
     * Парсит HTML и CSS из текстового файла с исходниками
     * @param {string} text Полное содержимое code.txt
     * @returns {{ html: string, css: string }}
     * @private
     */
    _parseInitialCodeFromText(text) {
        if (!text) return { html: '', css: '' };

        // Разделяем по закрывающему тегу </html>: до него — HTML, после — CSS
        const lower = text.toLowerCase();
        const htmlEndIdx = lower.indexOf('</html>');
        let htmlPart = '';
        let cssPart = '';
        if (htmlEndIdx !== -1) {
            htmlPart = text.slice(0, htmlEndIdx + 7);
            cssPart = text.slice(htmlEndIdx + 7);
        } else {
            // Если нет полного документа, считаем что CSS отсутствует, все — HTML
            htmlPart = text;
            cssPart = '';
        }

        // Извлекаем содержимое <body>...</body>
        let htmlContent = '';
        const bodyMatch = /<body[^>]*>([\s\S]*?)<\/body>/i.exec(htmlPart);
        if (bodyMatch && bodyMatch[1]) {
            htmlContent = bodyMatch[1].trim();
        } else {
            // Если <body> не найден, пробуем взять все как есть (на случай фрагмента)
            htmlContent = htmlPart.trim();
        }
...
<body>
     ...
     */
    _initGlobalCursors() {
        // Проверяем, не были ли уже инициализированы
        if (this.globalCursorsInitialized) {
            console.log('[AppInitializer] Глобальные курсоры уже инициализированы, пропускаем');
            return;
        }
        ...
        if (this.socketService && this.socketService.yAwareness && this.socketService.yDoc) {
            ...
            this.globalCursorsInitialized = true;
