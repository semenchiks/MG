    ...
    _subscribeToSocketEvents() {
        ...
        this.socketService.onAuth((data) => {
            if (data.success) {
                ...
                // Принудительно инициализируем CodeEditorManager после авторизации
                setTimeout(() => {
                    console.log('[AppInitializer] Принудительная инициализация после авторизации...');
                    
                    // Проверяем, что Yjs готов
                    if (this.socketService.yDoc && this.socketService.yAwareness) {
                        console.log('[AppInitializer] Yjs готов, инициализируем CodeEditorManager...');
                        this._initCodeEditorManager();
                        this._initGlobalCursors();
                    } else {
                        console.log('[AppInitializer] Yjs еще не готов, ждем...');
                        // Попробуем еще раз через секунду
                        setTimeout(() => {
                            if (this.socketService.yDoc && this.socketService.yAwareness) {
                                console.log('[AppInitializer] Yjs готов (вторая попытка), инициализируем CodeEditorManager...');
                                this._initCodeEditorManager();
                                this._initGlobalCursors();
                            } else {
                                console.error('[AppInitializer] Yjs так и не инициализировался');
                            }
                        }, 1000);
                    }
                }, 1000);
