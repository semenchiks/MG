...
import { SocketIOProvider } from './yjs-socketio-provider.js';
...
export class SocketService {
    ...
    _setupBeforeUnloadHandler() {
        window.addEventListener('beforeunload', () => {
            log('Закрытие окна браузера, очищаем соединения...');
    ...
    _getServerUrl() {
        ...
        // Vite прокси перенаправит запросы к нужному серверу
        if (window.location.port === '5173') {
            // Разработка через Vite - используем относительный путь для прокси
            return '';
    ...
    _getWebSocketUrl() {
        ...
        if (window.location.port === '5173') {
            // В режиме разработки через Vite
            // Определяем хост сервера автоматически
            ...
            if (isLocalhost) {
                ...
                // Подключение через IP - пытаемся найти IP хоста разработки
                // Поскольку Vite показывает Network адрес, используем его
                const devServerHost = this._getDevServerHost();
                return `${protocol}//${devServerHost}:3001`;
            }
        } else if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
            // В локальной разработке используем отдельный порт
            return `${protocol}//${window.location.hostname}:3001`;
        } else {
            // В продакшене используем тот же домен с путем /yjs
    ...
    /**
     * Получить хост dev сервера для WebSocket подключения
     * @private
     */
    _getDevServerHost() {
        // Пытаемся определить IP хоста разработки
        // В большинстве случаев это будет тот же IP что и у Vite сервера
        const currentHost = window.location.hostname;
        
        // Если это IP адрес, используем его
        if (currentHost.match(/^\d+\.\d+\.\d+\.\d+$/)) {
            return currentHost;
        }
        
        // Fallback - пытаемся угадать локальный IP
     ...
     * @private
     */
    async _initYjsConnection(roomName, isReset = false) {
        try {
            // Проверяем, есть ли уже активное соединение для этой комнаты
            if (this.yWebsocketProvider && !isReset && this.roomName === roomName) {
                log(`Yjs: Соединение для комнаты ${roomName} уже существует, пропускаем инициализацию`);
                return;
            }

            // Получаем информацию о WebSocket URL с сервера для диагностики
            try {
                const response = await fetch('/api/websocket-info');
                if (response.ok) {
                    const info = await response.json();
                    log(`WebSocket диагностика - ожидаемый URL: ${info.websocketUrl}, среда: ${info.environment}`);
                }
            } catch (fetchError) {
                log(`Не удалось получить диагностическую информацию: ${fetchError.message}`, 'warn');
            }

            // Закрываем существующее соединение если есть
            if (this.yWebsocketProvider) {
                log(`Yjs: Закрываем существующее соединение`);
                this.yWebsocketProvider.destroy();
                this.yWebsocketProvider = null;
            }

            log(`Yjs: Создаем новое соединение для комнаты: ${roomName}`);
            this.yDoc = new Y.Doc();
            ...
            if (window.location.port === '5173') {
                // В разработке через Vite используем WebSocket
                const wsUrl = this._getWebSocketUrl();
                log(`Yjs: Подключение к WebSocket серверу: ${wsUrl} для документа: ${roomName}`);
                log(`Yjs: Режим разработки через Vite, текущий хост: ${window.location.hostname}:${window.location.port}`);
                ...
                // Обработчики ошибок WebSocket
                this.yWebsocketProvider.on('connection-error', (error) => {
                    log(`Yjs WebSocket ошибка подключения: ${error}`, 'error');
                    console.error('[Yjs] Детали ошибки:', error);
                });

                this.yWebsocketProvider.on('connection-close', (event) => {
                    log(`Yjs WebSocket соединение закрыто: ${event.code} - ${event.reason}`, 'warn');
                });
            } else {
                // В продакшене или не через Vite используем Socket.IO
                log(`Yjs: Используем Socket.IO провайдер для документа: ${roomName}`);
                log(`Yjs: Текущий хост: ${window.location.hostname}:${window.location.port} (не Vite режим)`);
                
                this.yWebsocketProvider = new SocketIOProvider(this.socket, roomName, this.yDoc, {
                    awareness: new awarenessProtocol.Awareness(this.yDoc)
                });

                // Обработчики событий для Socket.IO провайдера
                this.yWebsocketProvider.on('status', (data) => {
                    log(`Yjs Socket.IO статус: ${data.status}`);
                    
                    // Когда соединение установлено, уведомляем о готовности
    ...
    _initYAwareness() {
        ...
        log(`Yjs Awareness: Инициализация для клиента ${localClientID}`);
        ...
        this.yAwareness.on('change', (changes) => {
            ...
            // Обновляем счетчик пользователей только если количество изменилось
    ...
    _sendPendingAuthRequest() {
        ...
        } else if (this.userName && this.roomName) {
            // Если пользователь уже был авторизован, просто переавторизуемся
            log('Переавторизация после переподключения...');
            this._performAuthorization(this.userName, null);
        }
    }
    ...
    _performAuthorization(userName, callback) {
        const authCallback = (response) => {
            if (response.success) {
                ...
                // Инициализируем Yjs соединение (только если его еще нет)
